...
export class InMemoryCollection {
  ...
  find(query: any = {}, options: any = {}): any[] {
    return this.documents.filter(doc =>
      ...
      this.matchParanoid(doc, options)
    );
  }
  ...
  findOneAndDelete(query: any): any {
    const index = this.documents.findIndex(doc => this.matchQuery(doc, query));
    if (index > -1) {
  ...
  deleteMany(query: any): { deletedCount: number; } {
    ...
    this.documents = this.documents.filter(doc => !this.matchQuery(doc, query));
  ...
  matchQuery(doc: any, query: any): boolean {
  ...
  matchParanoid(doc: any, options: any = {}): boolean {
    if (options?.paranoid === false) return true;
...
export const createMockMongooseModel = (mockMethods: IMockModel = {}, schema?: Schema): Model<any> => {
  ...
  const defaultMock = {
    ...
    countDocuments: jest.fn((query, options) => {
      const docs = collection.documents.filter(doc =>
        collection.matchQuery(doc, query)
      );
